...
function GridBackground() {
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none" }}>
      ...
      <div style={{ position: "absolute", inset: 0, background: `radial-gradient(ellipse 70% 50% at 20% 50%, rgba(29,78,216,0.22) 0%, transparent 60%), radial-gradient(ellipse 50% 70% at 80% 20%, rgba(109,40,217,0.18) 0%, transparent 55%), radial-gradient(ellipse 40% 40% at 60% 85%, rgba(6,182,212,0.1) 0%, transparent 50%)` }} />
      <div style={{ position: "absolute", inset: 0, backgroundImage: "radial-gradient(circle, rgba(99,102,241,0.18) 1px, transparent 1px)", backgroundSize: "32px 32px" }} />
      <div style={{ position: "absolute", inset: 0, backgroundImage: "repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(0,50,255,0.015) 2px, rgba(0,50,255,0.015) 4px)", mixBlendMode: "screen" }} />
    </div>
  );
}
...
function BrandPanel() {
  return (
    <div style={{ flex: 1, flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", padding: "64px 48px", overflow: "hidden" }} className="lg-brand-panel">
      <div style={{ position: "absolute", width: 480, height: 480, borderRadius: "50%", top: "50%", left: "50%", transform: "translate(-50%,-50%)", background: "radial-gradient(circle, rgba(37,99,235,0.2) 0%, rgba(109,40,217,0.12) 40%, transparent 70%)", filter: "blur(40px)", animation: "ambPulse 6s ease-in-out infinite" }} />
      {[
        ...
        <div key={i} style={{ position: "absolute", borderRadius: "50%", width: r.sz, height: r.sz, top: "50%", left: "50%", border: `1px solid ${r.bc}`, animation: `orbitSpin ${r.spd}s linear infinite ${r.rev ? "reverse" : "normal"}`, pointerEvents: "none" }}>
          <div style={{ position: "absolute", borderRadius: "50%", width: r.dot, height: r.dot, background: r.dc, boxShadow: `0 0 14px ${r.dc}`, top: `calc(-${r.dot / 2}px)`, left: "50%", transform: "translateX(-50%)" }} />
        </div>
      ))}
      <div style={{ position: "relative", zIndex: 10, display: "flex", flexDirection: "column", alignItems: "center", animation: "logoIn 1s cubic-bezier(.16,1,.3,1) both .3s" }}>
        <div style={{ position: "relative", marginBottom: 28, animation: "hexFloat 5s ease-in-out infinite" }}>
          <div style={{ position: "absolute", inset: -18, borderRadius: "50%", background: "radial-gradient(circle, rgba(37,99,235,0.6) 0%, transparent 65%)", filter: "blur(18px)", animation: "ambPulse 3s ease-in-out infinite" }} />
        ...
        <p style={{ fontSize: 14, fontWeight: 300, textAlign: "center", lineHeight: 1.8, maxWidth: 240, color: "rgba(180,210,255,0.5)", opacity: 0, animation: "fadeUp .7s ease both 1.4s" }}>Secure, intelligent banking <br />always at your fingertips.</p>
      ...
      {steps.map((lbl, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div style={{ width: 26, height: 26, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, transition: "all 0.3s", background: step === i+1 ? "linear-gradient(135deg,#4f46e5,#7c3aed)" : step > i+1 ? "linear-gradient(135deg,#10b981,#059669)" : "rgba(255,255,255,0.08)", color: (step === i+1 || step > i+1) ? "#fff" : "rgba(255,255,255,0.3)", boxShadow: step === i+1 ? "0 4px 12px rgba(79,70,229,0.5)" : "none" }}>{step > i+1 ? "✓" : i+1}</div>
            <span style={{ fontSize: 11, fontWeight: 500, color: step === i+1 ? "#e2e8f0" : "rgba(148,163,184,0.38)", transition: "color 0.3s" }}>{lbl}</span>
          </div>
          {i < 2 && <div style={{ width: 20, height: 2, borderRadius: 99, transition: "background 0.4s", background: step > i+1 ? "linear-gradient(90deg,#10b981,#059669)" : "rgba(255,255,255,0.1)" }} />}
        </div>
      ))}
    </div>
  );
      ...
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden", padding: "24px 16px", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        ...
        <div style={{ position: "relative", zIndex: 10, display: "flex", alignItems: "center", width: "100%", maxWidth: 960 }}>
          ...
          <div style={{ width: "100%", maxWidth: 460, animation: "cardIn .8s cubic-bezier(.16,1,.3,1) both .1s" }}>
            <div style={{ borderRadius: 24, background: "rgba(8,16,60,0.7)", border: "1px solid rgba(99,102,241,0.2)", backdropFilter: "blur(32px)", boxShadow: "0 0 0 1px rgba(255,255,255,0.03) inset, 0 24px 80px rgba(0,0,0,0.6), 0 0 100px rgba(37,99,235,0.12)", padding: "36px 36px 32px" }}>

              {/* ❌ Top bar REMOVED (PayZen Bank + Secure badge) */}
              ...
              {step === 2 && (
                <div style={{ animation: "stepIn .35s cubic-bezier(.16,1,.3,1) forwards" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "center", gap: 8, marginBottom: 14 }}>
                    {otp.map((d, i) => (
                      <div key={i} onClick={() => refs[i].current.focus()} style={{ position: "relative", width: 46, height: 52, cursor: "text" }}>
                        <div style={{ position: "absolute", inset: 0, borderRadius: 12, background: focusIdx === i ? "rgba(99,102,241,0.08)" : "rgba(255,255,255,0.04)", border: `1px solid ${focusIdx === i ? "rgba(99,102,241,0.6)" : "rgba(99,102,241,0.2)"}`, boxShadow: focusIdx === i ? "0 0 0 3px rgba(99,102,241,0.12)" : "none", transition: "all 0.2s", display: "flex", alignItems: "center", justifyContent: "center" }}>
                          {d ? <div style={{ width: 9, height: 9, borderRadius: "50%", background: "#e2e8f0", boxShadow: "0 0 6px rgba(226,232,240,0.6)" }} /> : focusIdx === i ? <div style={{ width: 2, height: 20, background: "#818cf8", borderRadius: 1, animation: "cursorBlink 1s step-end infinite" }} /> : null}
                        </div>
                  ...
                  <p style={{ textAlign: "center", marginBottom: 14 }}>
                    <button onClick={() => { setStep(1); setOtp(["","","","","",""]); setErr(""); }} style={{ background: "none", border: "none", cursor: "pointer", color: "#818cf8", fontSize: 12, fontFamily: "inherit" }}>
                    ...
                    <div>
                      ...
                      {conP && (
                        <div style={{ display: "flex", alignItems: "center", gap: 5, marginTop: 7 }}>
                          ...
                          <span style={{ fontSize: 10, color: pwMatch ? "#4ade80" : "#f87171" }}>{pwMatch ? "Passwords match ✓" : "Doesn't match"}</span>
                        </div>
                      )}
                    </div>
              ...
              {/* Error / Success */}
              {err && <div style={{ marginTop: 14, padding: "10px 14px", borderRadius: 10, background: "rgba(239,68,68,0.07)", border: "1px solid rgba(239,68,68,0.25)", color: "#fca5a5", fontSize: 13, textAlign: "center" }}>{err}</div>}
              {suc && <div style={{ marginTop: 14, padding: "10px 14px", borderRadius: 10, background: "rgba(52,211,153,0.07)", border: "1px solid rgba(52,211,153,0.25)", color: "#6ee7b7", fontSize: 13, textAlign: "center" }}>{suc}</div>}
              ...
              {step > 1 && (
                <div style={{ textAlign: "center", marginTop: 12 }}>
                  <button onClick={() => { setStep(step - 1); setErr(""); setSuc(""); }} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(148,163,184,0.5)", fontSize: 12, fontFamily: "inherit" }}>
                    ← Back
                  </button>
                </div>
              )}
