...
export default function Setting() {
  ...
  const [profile, setProfile] = useState({
    name:    { label: "Full Name",      value: "Bhushan Patil",            type: "text",  icon: P.user,   color: "text-blue-600",   bg: "bg-blue-50"   },
    email:   { label: "Email Address",  value: "bhushan@example.com",      type: "email", icon: P.mail,   color: "text-violet-600", bg: "bg-violet-50" },
    phone:   { label: "Phone Number",   value: "+91 98765 43210",          type: "tel",   icon: P.phone,  color: "text-green-600",  bg: "bg-green-50"  },
    dob:     { label: "Date of Birth",  value: "1995-06-15",               type: "date",  icon: P.cal,    color: "text-orange-500", bg: "bg-orange-50" },
  ...
  const tabs = [
    ...
    { id: "help",          label: "Help & Support",   icon: P.help   },
  ];

 

  /* ══ FULL PAGE DETAIL EDIT (field click) ══════════════════════ */
  if (detailKey) {
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="w-full max-w-md rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="px-6 py-7">
            ...
            />
            <p className="text-xs text-slate-400 mt-2">Current: <span className="text-slate-600 font-semibold">{f.value}</span></p>
            <div className="mt-7 flex gap-3">
              <button onClick={() => setDetailKey(null)} className="flex-1 py-2.5 rounded-xl border-2 border-slate-200 text-slate-600 text-sm font-semibold hover:bg-slate-50 transition-colors">Cancel</button>
              <button
                onClick={saveDetail}
                className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${fieldSaved ? "bg-green-500 text-white" : "bg-blue-600 hover:bg-blue-700 text-white"}`}
              >
  ...
  return (
    <div className="min-h-screen bg-[#f0f4f8] flex flex-col">
      ...
      <div className="flex flex-1 gap-5 px-5 py-6 max-w-[1200px] w-full mx-auto">


        {/* ── CONTENT ──────────────────────────────────────── */}
        <main className="flex-1 min-w-0 flex flex-col gap-4">

          {/* Tab bar — top right aligned like wireframe */}
          ...
          {tab === "profile" && (
            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
              ...
              <div className="px-7 py-4 border-t border-slate-100 flex items-center justify-between bg-slate-50/60">
                <p className="text-xs text-slate-400 flex items-center gap-1.5">
                  <Ico path={P.shield} size={12} cls="text-slate-400" /> Your data is encrypted and secure.
                </p>
                <button
                  onClick={saveProfile}
                  className={`px-5 py-2 rounded-xl text-xs font-black transition-all ${profileSaved ? "bg-green-500 text-white" : "bg-blue-600 hover:bg-blue-700 text-white"}`}
                >
          ...
          {tab === "security" && (
            <div className="flex flex-col gap-4">
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                ...
                {showPwd && (
                  <div className="px-7 pb-6 border-t border-slate-100 pt-5">
                    {pwdMsg && (
                      <div className={`mb-5 px-4 py-2.5 rounded-xl text-xs font-semibold flex items-center gap-2 ${pwdMsg.ok ? "bg-green-50 text-green-700 border border-green-200" : "bg-red-50 text-red-600 border border-red-200"}`}>
                        <Ico path={pwdMsg.ok ? P.check : P.lock} size={13} />
                        {pwdMsg.text}
                      </div>
                    )}
                    <div className="grid grid-cols-3 gap-4">
                      {[{ k: "old", l: "Current Password" }, { k: "new", l: "New Password" }, { k: "confirm", l: "Confirm Password" }].map(({ k, l }) => (
                        <div key={k}>
                          ...
                          <input
                            ...
                            placeholder="••••••••"
                            className="w-full border-2 border-slate-200 focus:border-blue-500 rounded-xl px-3 py-2.5 text-sm bg-white focus:outline-none transition-all"
                          />
                        </div>
                      ))}
                    </div>
                    <button
                      onClick={updatePwd}
                      className="mt-4 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-black rounded-xl transition-all"
                    >
                      Update Password
                    </button>
                  </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
              ...
              <div className="divide-y divide-slate-100">
                {Object.entries(notifs).map(([key, item]) => (
                  <div key={key} className="px-7 py-4 flex items-center justify-between hover:bg-slate-50/60 transition-colors">
                    <div className="flex items-center gap-4">
              ...
              <div className="px-7 py-4 border-t border-slate-100 flex justify-end bg-slate-50/60">
                <button
                  onClick={saveNotif}
                  className={`px-5 py-2 rounded-xl text-xs font-black transition-all flex items-center gap-2 ${notifSaved ? "bg-green-500 text-white" : "bg-blue-600 hover:bg-blue-700 text-white"}`}
                >
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                ...
                <div className="grid grid-cols-2 gap-4 p-5">
                  {[
                    {
                      icon: P.phone, label: "Call Us",
                      val: "1800-11-2211", sub: "Toll Free · 24/7",
                      iconBg: "bg-blue-600", tx: "text-blue-600",
                    },
                    {
                      icon: P.mail, label: "Email Support",
                      val: "support@payzen.in", sub: "Reply within 24 hrs",
                      iconBg: "bg-emerald-500", tx: "text-emerald-600",
                    },
                    {
                      icon: P.sms, label: "Live Chat",
                      val: "Talk to an agent", sub: "Avg wait < 2 min",
                      iconBg: "bg-violet-600", tx: "text-violet-600",
                    },
                    {
                      icon: P.map, label: "Find Branch",
                      val: "Nearest branch", sub: "View on map",
                      iconBg: "bg-rose-500", tx: "text-rose-600",
                    },
                  ].map((c, i) => (
                    <button key={i} className="text-left p-5 rounded-xl border border-slate-100 hover:border-slate-200 hover:shadow-sm transition-all group">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                ...
                <div className="divide-y divide-slate-100">
                  {[
                    ...
                    <button key={i} className="w-full px-7 py-4 flex items-center justify-between text-left hover:bg-slate-50 transition-colors group">
                      <span className="text-sm font-semibold text-slate-700 group-hover:text-blue-700 transition-colors">{q}</span>
                      <span className="text-slate-300 group-hover:text-blue-500 transition-colors ml-4 shrink-0">
